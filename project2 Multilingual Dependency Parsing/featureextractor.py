    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if stack:
            ...
            if FeatureExtractor._check_informative(token['ctag'],True):
                result.append('STK_0_CTAG' + token['ctag'])
            if len(stack)>=2:
                stack_idx1 = stack[-2]
                token2 = tokens[stack_idx1]
                if FeatureExtractor._check_informative(token2['tag'], True):
                    result.append('STK_1_TAG_' + token2['tag'])
                if FeatureExtractor._check_informative(token2['ctag'],True):
                    result.append('STK_1_CTAG' + token2['ctag'])
        ...
        if buffer:
            ...
            if FeatureExtractor._check_informative(token['tag'],True):
                result.append('BUF_0_TAG' + token['tag'])
            if FeatureExtractor._check_informative(token['ctag'],True):
                result.append('BUF_0_CTAG' + token['ctag'])
            if len(buffer)>=2:
                buffer_idx3  = buffer[1]
                token_two = tokens[buffer_idx3]
                if FeatureExtractor._check_informative(token_two['tag'],True):
                    result.append('BUF_1_TAG' + token_two['tag'])
